...
\frac{\partial\rho{E}}{\partial{t}}+\frac{\partial\rho{u}_{k}E}{\partial{x}_{k}}=&-\frac{\partial{p}u_{k}}{\partial{x}_{k}}-\frac{\partial{q}_{k}}{\partial{x}_{k}}+\frac{\partial\tau_{ki}u_{i}}{\partial{x}_{k}}+{u}_{k}f_{k}\label{eq:en}\\
...
\begin{equation}-\rho{V}_{\alpha,k}Y_{\alpha}=\rho{D}_{\alpha}\frac{\partial{Y}_{\alpha}}{\partial{x}_{k}}-Y_{\alpha}\displaystyle\sum_{\beta\in\left[1,N_{S}\right]}\rho{D}_{\beta}\frac{\partial{Y}_{\beta}}{\partial{x}_{k}}\label{eq:fickdiff},\end{equation}
...
Derivatives of the conservative variables $\rho$, $\rho{u}_{i}$, $\rho{E}$ and $\rho{Y}_{\alpha}$, and also the temperature and pressure,  are evaluated directly with the LABFM method~\cite{king_labfm_2022} in the $x-y$ plane, and central finite differences in $z$. To avoid the explicit evaluation of cross-second derivatives of velocity, the viscous stress divergence is evaluated as
...
where the final term is the gradient of the velocity divergence, which is calculated through two iterations of first derivative evalations. Laplacians are also evaluated directly using LABFM combined with finite differences. This formulation permits smaller computational stencils, and reduces the amount of MPI communication required.
...
\begin{equation}\frac{\partial\rho{u}_{i}}{\partial{t}}+\rho{u}_{k}\frac{\partial{u}_{i}}{\partial{x}_{k}}+u_{i}\frac{\partial\rho{u}_{k}}{\partial{x}_{k}}=-\frac{\partial{p}}{\partial{x}_{i}}+\mu\left[\frac{\partial^{2}u_{i}}{\partial{x}_{k}\partial{x}_{k}}-\frac{1}{3}\frac{\partial}{\partial{x}_{i}}\left(\frac{\partial{u}_{k}}{\partial{x}_{j}}\right)\right]+\left[\frac{\partial{u}_{i}}{\partial{x}_{k}}+\frac{\partial{u}_{k}}{\partial{x}_{i}}-\frac{2}{3}\delta_{ik}\frac{\partial{u}_{j}}{\partial{x}_{k}}\right]\frac{\partial\mu}{\partial{x}_{j}}+\rho{f}_{i}\end{equation}
