...
contains
  subroutine calc_fd_weights
     ...
     ijco2 = 7
#else
     write(6,*) "Warning: FDORDER has wrong value. Stopping."
#ifdef mp
     ...
     do i=1,npfb
        iz = zlayer_index(i)         !! iz is the z-index of node i
        il = i - npfb_layer*(iz-1)   !! il is the index of node i within the iz-th layer

        !! Loop over neighbours     
        do jj = 1,ij_count_fd
           !! Work out which layer the jj-th neighbour is in
           j = iz + jj - ijco2
           if(j.le.0) j = j + nz
           if(j.gt.nz) j = j - nz
           ij_link_fd(jj,i) = npfb_layer*(j-1) + il !! store the index of this neighbour
        end do   
     end do
     !$omp end parallel do
